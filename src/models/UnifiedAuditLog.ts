...
const UnifiedAuditLogSchema = new Schema<IUnifiedAuditLog>({
  ...
  targetResource: {
    type: {
      ...
      required: function(this: IUnifiedAuditLog) {
...
UnifiedAuditLogSchema.index({ actorId: 1, timestamp: -1 });
UnifiedAuditLogSchema.index({ category: 1, timestamp: -1 });
UnifiedAuditLogSchema.index({ action: 1, outcome: 1 });
UnifiedAuditLogSchema.index({ 'targetResource.type': 1, 'targetResource.id': 1 });
UnifiedAuditLogSchema.index({ 'securityContext.riskLevel': 1, timestamp: -1 });
UnifiedAuditLogSchema.index({ 'securityContext.isSensitive': 1, 'securityContext.requiresReview': 1 });
UnifiedAuditLogSchema.index({ 'requestInfo.ipAddress': 1, timestamp: -1 });
UnifiedAuditLogSchema.index({ 'requestInfo.sessionId': 1, timestamp: -1 });

// TTL index for automatic cleanup (2 years default)
UnifiedAuditLogSchema.index({ timestamp: 1 }, { expireAfterSeconds: 63072000 });

// Static methods for common operations
UnifiedAuditLogSchema.statics.logAction = async function(logData: Partial<IUnifiedAuditLog>) {
  const log = new this(logData);
  return await log.save();
};

// Get recent activity
UnifiedAuditLogSchema.statics.getRecentActivity = function(
  limit: number = 50,
  actorId?: string,
...
UnifiedAuditLogSchema.statics.getActivityByCategory = function(
  category: AuditCategory,
  startDate?: Date,
  endDate?: Date,
  limit: number = 100
...
// Get failed actions
UnifiedAuditLogSchema.statics.getFailedActions = function(limit: number = 100) {
...
UnifiedAuditLogSchema.statics.getSensitiveActions = function(limit: number = 100) {
...
UnifiedAuditLogSchema.statics.getHighRiskActivities = function(limit: number = 100) {
  return this.find({ 
...
UnifiedAuditLogSchema.statics.getActionsRequiringReview = function(limit: number = 100) {
...
// Get user activity
UnifiedAuditLogSchema.statics.getUserActivity = function(
  userId: string,
  limit: number = 50,
  startDate?: Date,
  endDate?: Date
...
// Get admin activity
UnifiedAuditLogSchema.statics.getAdminActivity = function(
  adminId: string,
  limit: number = 50,
  startDate?: Date,
  endDate?: Date
...
// Create or get the UnifiedAuditLog model
let UnifiedAuditLog: Model<IUnifiedAuditLog>;

try {
  if (mongoose.models.UnifiedAuditLog) {
    UnifiedAuditLog = mongoose.models.UnifiedAuditLog as Model<IUnifiedAuditLog>;
    console.log('📋 Models: Using existing UnifiedAuditLog model');
  } else {
    UnifiedAuditLog = mongoose.model<IUnifiedAuditLog>('UnifiedAuditLog', UnifiedAuditLogSchema);
    console.log('📋 Models: UnifiedAuditLog model created successfully');
  }
} catch (error) {
  console.log('📋 Models: Fallback - creating new UnifiedAuditLog model');
  UnifiedAuditLog = mongoose.model<IUnifiedAuditLog>('UnifiedAuditLog', UnifiedAuditLogSchema);
}

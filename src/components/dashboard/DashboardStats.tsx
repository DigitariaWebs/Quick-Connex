...
interface DashboardStatsProps {
  ...
  onFilterChange: (filter: 'all' | 'pending' | 'accepted' | 'in_progress' | 'completed') => void;
...
const StatCard = ({ 
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`relative overflow-hidden rounded-2xl shadow-md bg-white cursor-pointer transition-all duration-200 ${
        isActive ? 'ring-2 ring-offset-2 ring-indigo-500' : ''
      }`}
    >
      {/* Gradient Background */}
      <div className={`absolute inset-0 opacity-${isActive ? '20' : '10'} ${gradient}`}></div>
...
export default function DashboardStats({ stats, onFilterChange, currentFilter }: DashboardStatsProps) {
  const statCards = [
    {
      title: 'All Requests',
      value: stats.totalPending + stats.totalAccepted + stats.totalInProgress + stats.totalCompleted,

...
export default function EmployeeDashboard() {
  ...
  const fetchTransfers = async () => {
    try {
      ...
      // For completed status, we need to handle it specially since the API might not have this filter
  ...
  const calculateStats = (transferData: TransferRequest[]) => {
    const newStats = {
      ...
      totalInProgress: transferData.filter(t => t.status === 'in_progress').length,
      totalCompleted: transferData.filter(t => t.status === 'completed').length
    };
    setStats(newStats);
  };
  ...
  const handleFilterChange = (newFilter: 'all' | 'pending' | 'accepted' | 'in_progress' | 'completed') => {
    setFilter(newFilter);
  ...
  if (priorityFilter) {
    filteredTransfers = filteredTransfers.filter(t => t.priority === priorityFilter);
  }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="flex items-center">
              ...
              <div className="ml-3 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                {filteredTransfers.length} request{filteredTransfers.length !== 1 ? 's' : ''}
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="pl-9 pr-4 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent w-48 md:w-64"
                />

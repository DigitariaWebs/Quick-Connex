  ...
  try {
    ...
    if (user.userType !== "employee") {
      return NextResponse.json(
        { error: "Only employees can access their accepted transfers" },
        { status: 403 },
      );
    }
    ...
    if (status && status !== "all") {
      ...
      // Default: only show active transfers (not pending, accepted, or cancelled)
      query.status = {
        $in: [TransferStatus.IN_PROGRESS, TransferStatus.COMPLETED],
      };
    }

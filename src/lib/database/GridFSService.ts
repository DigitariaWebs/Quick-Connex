...
export class GridFSService {
  ...
  static validateFileBuffer(
    ...
    // Check file size
    const maxSizeBytes = MAX_FILE_SIZES[mimeType as keyof typeof MAX_FILE_SIZES] || MAX_FILE_SIZES.default;
    if (size > maxSizeBytes) {
      return {
        isValid: false,
        errors: [`File size must be less than ${Math.round(maxSizeBytes / 1024 / 1024)}MB`],
        warnings: []
      };
    }
    ...
    if (!ALLOWED_DOCUMENT_TYPES.includes(mimeType)) {
      return {
        isValid: false,
        errors: [`File type not allowed. Allowed types: ${ALLOWED_DOCUMENT_TYPES.join(', ')}`],
        warnings: []
      };
    }

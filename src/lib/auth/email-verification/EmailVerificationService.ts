...
export class EmailVerificationService {
  ...
  static async sendVerificationCode(
    ...
    try {
      ...
      const recentCodeCount = await EmailVerification.countRecentCodes(
        normalizedEmail,
        60,
      );
      console.log(
        "📧 [EmailVerification] Recent code count (from static method):",
        recentCodeCount,
      );
      ...
      console.log(
        "📧 [EmailVerification] Invalidating existing unverified codes...",
      );
      ...
      console.log(
        "📧 [EmailVerification] Verification record created:",
        verification._id,
      );

      // Send email via CommunicationService
      console.log(
        "📧 [EmailVerification] Getting CommunicationService instance...",
      );
      const communicationService = CommunicationService.getInstance();
      console.log(
        "📧 [EmailVerification] CommunicationService instance obtained",
      );
      ...
      const emailTemplate = templateLoader.renderTemplate(
        "email/auth/email-verification.html",
        templateData,
      );
      ...
      if (!emailResult.success) {
        console.log(
          "❌ [EmailVerification] Email failed, deleting verification record...",
        );
        // Delete verification record if email failed
        await DatabaseService.deleteOne(EmailVerification, {
      ...
      console.error(
        "❌ [EmailVerification] Error in sendVerificationCode:",
        error,
      );
      console.error(
        "❌ [EmailVerification] Error type:",
      ...
      console.error(
        "❌ [EmailVerification] Error stack:",
        error instanceof Error ? error.stack : "No stack",
      );

      if (error instanceof ValidationError || error instanceof AppError) {
        console.log(
          "📧 [EmailVerification] Re-throwing ValidationError/AppError",
        );
        throw error;
      }
  ...
  static async verifyCode(
    ...
    try {
      ...
      const verification =
        (await EmailVerification.findActiveVerification(normalizedEmail)) ||
  ...
  static async isEmailVerified(
    ...
    try {
      const normalizedEmail = normalizeEmail(email);
      const verified = await EmailVerification.findVerifiedEmail(
        normalizedEmail,
        withinMinutes,
      );
      return !!verified;
  ...
  static async getVerificationStatus(email: string): Promise<{
    ...
    try {
      ...
      // Check rate limiting
      const recentCodes = await EmailVerification.countRecentCodes(
        normalizedEmail,
        60,
      );
      const codesCount =

...
import PhoneVerification, { IPhoneVerification } from '@/models/PhoneVerification';
...
export class PhoneVerificationService {
  ...
  static async sendVerificationCode(
    ...
    try {
      ...
      const recentCodeCount = await PhoneVerification.countRecentCodes(normalizedPhone, 60);
      console.log('📱 [PhoneVerification] Recent code count (from static method):', recentCodeCount);
      
      const count = typeof recentCodeCount === 'number' ? recentCodeCount : await PhoneVerification.countDocuments({
      ...
      console.log('📱 [PhoneVerification] Invalidating existing unverified codes...');
      ...
      console.log('📱 [PhoneVerification] Verification record created:', verification._id);
      
      // Send SMS via CommunicationService
      console.log('📱 [PhoneVerification] Getting CommunicationService instance...');
      const communicationService = CommunicationService.getInstance();
      console.log('📱 [PhoneVerification] CommunicationService instance obtained');
      ...
      if (!smsResult.success) {
        console.log('❌ [PhoneVerification] SMS failed, deleting verification record...');
        // Delete verification record if SMS failed
        await DatabaseService.deleteOne(PhoneVerification, { _id: verification._id });
      ...
      console.error('❌ [PhoneVerification] Error in sendVerificationCode:', error);
      console.error('❌ [PhoneVerification] Error type:', error?.constructor?.name);
      console.error('❌ [PhoneVerification] Error message:', error instanceof Error ? error.message : String(error));
      console.error('❌ [PhoneVerification] Error stack:', error instanceof Error ? error.stack : 'No stack');
      
      if (error instanceof ValidationError || error instanceof AppError) {
        console.log('📱 [PhoneVerification] Re-throwing ValidationError/AppError');
        throw error;
      }
  ...
  static async verifyCode(
    ...
    try {
      ...
      const verification = await PhoneVerification.findActiveVerification(normalizedPhone) || 
   ...
   */
  static async isPhoneVerified(phone: string, withinMinutes: number = 10): Promise<boolean> {
    try {
      const normalizedPhone = normalizePhoneNumber(phone);
      const verified = await PhoneVerification.findVerifiedPhone(normalizedPhone, withinMinutes);
      return !!verified;
    } catch (error) {
  ...
  static async getVerificationStatus(phone: string): Promise<{
    ...
    try {
      ...
      // Check if verified
      const verified = await PhoneVerification.findVerifiedPhone(normalizedPhone, 10);
      
      // Check rate limiting
      const recentCodes = await PhoneVerification.countRecentCodes(normalizedPhone, 60);
      const codesCount = typeof recentCodes === 'number' ? recentCodes : await PhoneVerification.countDocuments({
        phone: normalizedPhone,
        createdAt: { $gte: new Date(Date.now() - 60 * 60 * 1000) }
      });

...
export function validateTransferData(data: any): TransferValidationResult {
  ...
  if (
    ...
    !validCategories.includes(data.transferCategory)
  ) {
    errors.push(
      "Transfer category is required and must be one of: patient, envelope, medical_instruments",
    );
  }
  ...
  if (data.transferCategory === "patient") {
    ...
    for (const field of medicalFields) {
      if (
        ...
        errors.push(
          `${fieldNames[field]} is required for medical instruments transfers`,
        );
      }
    }
  }
  ...
  if (
    fromHospitalValue &&
    toHospitalValue &&
    fromHospitalValue === toHospitalValue
  ) {
    errors.push("From and to hospitals cannot be the same");
  }

  // Date validation (day-granularity to avoid timezone-driven false negatives on today)
  ...
  if (data.reason && data.reason.length < 10) {
    warnings.push(
      "Transfer reason is quite short. Consider providing more details.",
    );
  }

  // Dossier number validation (optional field)
  if (
    data.patientDossierNumber &&
    data.patientDossierNumber.toString().trim() !== ""
  ) {

...
/**
 * Test Email Notification Flow Script
 * 
 * This script tests the complete email notification flow
 * to ensure all data is properly passed through the system.
 */
...
async function testEmailNotificationFlow() {
    try {
        ...
        // Find a transfer and populate it the same way as in the API
        ...
        console.log('👤 Requested By User (from populated transfer):');
        ...
        // Simulate the TransferNotificationService.sendNewTransferRequestNotification call
        console.log('📧 Simulating TransferNotificationService.sendNewTransferRequestNotification...');

        // This is the exact same logic as in the TransferNotificationService
        const transferData = {
        ...
        console.log('📧 Transfer Data Object (from notification service):');
        ...
        // Test the email generation methods
        console.log('\n📧 Testing Email Generation Methods:');
        console.log('='.repeat(60));

        // Simulate generateTransferRequestEmailText
        const textEmail = `
...
Please review and respond to this transfer request as soon as possible.
    `.trim();
        ...
        console.log('📝 Generated Text Email:');
        console.log(textEmail);

        // Check for undefined values in the generated email
        console.log('\n🔍 Email Content Validation:');
        console.log('='.repeat(60));

        const issues = [];

        if (textEmail.includes('undefined')) {
            issues.push('❌ Email contains "undefined" values');
        } else {
            console.log('✅ No "undefined" values found in email');
        }

        if (!textEmail.includes(transferData.patientDossier) || transferData.patientDossier === 'undefined') {
            issues.push('❌ Dossier Number is missing or undefined in email');
        } else {
            console.log('✅ Dossier Number is present in email');
        }

        if (!textEmail.includes(transferData.requestedByEmail) || transferData.requestedByEmail === 'undefined') {
            issues.push('❌ Requestor Email is missing or undefined in email');
        } else {
            console.log('✅ Requestor Email is present in email');
        }

        if (!textEmail.includes(transferData.requestedByPhone) || transferData.requestedByPhone === 'undefined') {
            issues.push('❌ Requestor Phone is missing or undefined in email');
        } else {
            console.log('✅ Requestor Phone is present in email');
        }
        ...
        if (issues.length > 0) {
            ...
            console.log('\n✅ All email content is properly generated!');
        }

        console.log('\n🎯 Test Summary:');
        console.log('✅ Transfer data populated correctly');
        console.log('✅ Notification service data object created successfully');
        console.log('✅ Email generation methods work correctly');
        console.log('✅ All required fields are present in generated emails');
...
module.exports = { testEmailNotificationFlow };

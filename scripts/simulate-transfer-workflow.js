...
/**
 * Transfer System Simulation Script
 * 
 * This script simulates the complete transfer workflow:
 ...
 * 
 * Includes rate limiting considerations for login attempts.
 * 
 * Usage: node scripts/simulate-transfer-workflow.js
 */
...
// Rate limiting configuration - be careful with login attempts
const RATE_LIMIT_DELAY = 2000; // 2 seconds between login attempts
...
const TEST_TRANSFER = {
    ...
    priority: 'high',
    reason: 'Specialized cardiac surgery required - patient has complex coronary artery disease',
    notes: 'Patient requires immediate transfer for urgent cardiac procedure. Monitor vitals closely during transport.'
};
...
async function loginUser(email, password, isManager = false) {
    ...
    log(`Waiting ${RATE_LIMIT_DELAY}ms to respect rate limits...`, 'warning');
...
async function createTransferRequest() {
    log('Creating transfer request as manager...', 'info');
...
async function runTransferSimulation() {
    try {
        log('Starting Transfer System Simulation', 'info');
        log(`Base URL: ${BASE_URL}`, 'info');
        log(`Rate limit delay: ${RATE_LIMIT_DELAY}ms between logins`, 'warning');
        ...
        log(`Patient: ${transferToReview.patientInfo.firstName} ${transferToReview.patientInfo.lastName}`, 'info');
        ...
        log(`Patient: ${finalTransfer.patientInfo.firstName} ${finalTransfer.patientInfo.lastName}`, 'info');
        log(`Status: ${finalTransfer.status}`, 'info');
        log(`Requested by: ${finalTransfer.issuer}`, 'info');
        log(`Created: ${finalTransfer.createdAt}`, 'info');
        log(`Completed: ${finalTransfer.updatedAt}`, 'info');

        // Calculate duration
        const duration = new Date(finalTransfer.updatedAt) - new Date(finalTransfer.createdAt);
        const durationMinutes = Math.round(duration / (1000 * 60));
        log(`Total Duration: ${durationMinutes} minutes`, 'info');

        log('\n🎉 TRANSFER SIMULATION COMPLETED SUCCESSFULLY! 🎉', 'success');

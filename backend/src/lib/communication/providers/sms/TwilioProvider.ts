  ...
  async send(message: SMSMessage): Promise<CommunicationServiceResponse> {
    try {
      log.debug('TwilioProvider sending SMS', 
        createCommunicationContext('twilio_send_start', {
          provider: 'twilio',
      ...
      if (!validation.isValid) {
        log.warn('TwilioProvider message validation failed', 
          createCommunicationContext('twilio_validation_failed', {
            provider: 'twilio',
            messageId: message.id,
      ...
      log.debug('TwilioProvider SMS sent successfully', 
        createCommunicationContext('twilio_send_success', {
          provider: 'twilio',
          messageId: message.id,
      ...
      log.error('TwilioProvider send error', 
        createCommunicationContext('twilio_send_error', {
          provider: 'twilio',
          messageId: message.id,
  ...
  async getStatus(messageId: string): Promise<CommunicationStatus> {
    try {
      log.debug('TwilioProvider checking message status', 
        createCommunicationContext('twilio_status_check', {
          provider: 'twilio',
          messageId
        })
      );
      ...
      log.debug('TwilioProvider status retrieved', 
        createCommunicationContext('twilio_status_retrieved', {
          provider: 'twilio',
          messageId,
      ...
      log.error('TwilioProvider status check error', 
        createCommunicationContext('twilio_status_error', {
          provider: 'twilio',
          messageId,
          error: error instanceof Error ? error.message : 'Unknown error'
        })
  ...
  async validateConfiguration(): Promise<boolean> {
    try {
      log.debug('TwilioProvider validating configuration', 
        createCommunicationContext('twilio_config_validation', {
          provider: 'twilio'
        })
      );
      ...
      if (!this.config.accountSid || !this.config.authToken) {
        log.warn('TwilioProvider missing credentials', 
          createCommunicationContext('twilio_config_missing_credentials', {
            provider: 'twilio'
          })
        );
        return false;
      ...
      log.debug('TwilioProvider configuration valid', 
        createCommunicationContext('twilio_config_valid', {
          provider: 'twilio',
      ...
      log.error('TwilioProvider configuration validation error', 
        createCommunicationContext('twilio_config_error', {
          provider: 'twilio',
          error: error instanceof Error ? error.message : 'Unknown error'
        })
      );

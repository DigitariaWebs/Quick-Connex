  ...
  async send(message: EmailMessage): Promise<CommunicationServiceResponse> {
    try {
      log.debug('SendGridProvider sending email', 
        createCommunicationContext('sendgrid_send_start', {
          provider: 'sendgrid',
      ...
      if (!validation.isValid) {
        log.warn('SendGridProvider message validation failed', 
          createCommunicationContext('sendgrid_validation_failed', {
            provider: 'sendgrid',
            messageId: message.id,
      ...
      log.debug('SendGridProvider email sent successfully (stub)', 
        createCommunicationContext('sendgrid_send_success', {
          provider: 'sendgrid',
          messageId: message.id
        })
      );
      ...
      log.error('SendGridProvider send error', 
        createCommunicationContext('sendgrid_send_error', {
          provider: 'sendgrid',
          messageId: message.id,
  ...
  async getStatus(messageId: string): Promise<CommunicationStatus> {
    try {
      log.debug('SendGridProvider checking message status', 
        createCommunicationContext('sendgrid_status_check', {
          provider: 'sendgrid',
          messageId
        })
      );
      ...
      log.error('SendGridProvider status check error', 
        createCommunicationContext('sendgrid_status_error', {
          provider: 'sendgrid',
          messageId,
          error: error instanceof Error ? error.message : 'Unknown error'
        })
  ...
  async validateConfiguration(): Promise<boolean> {
    try {
      log.debug('SendGridProvider validating configuration', 
        createCommunicationContext('sendgrid_config_validation', {
          provider: 'sendgrid'
        })
      );
      ...
      if (!this.config.apiKey) {
        log.warn('SendGridProvider missing API key', 
          createCommunicationContext('sendgrid_config_missing_api_key', {
            provider: 'sendgrid'
          })
        );
        return false;
      ...
      log.debug('SendGridProvider configuration valid', 
        createCommunicationContext('sendgrid_config_valid', {
          provider: 'sendgrid'
        })
      );
      return true;
    } catch (error) {
      log.error('SendGridProvider configuration validation error', 
        createCommunicationContext('sendgrid_config_error', {
          provider: 'sendgrid',
          error: error instanceof Error ? error.message : 'Unknown error'
        })
      );

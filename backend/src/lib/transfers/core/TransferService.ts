...
export interface TransferServiceResponse<T = any> {
  success: boolean;
  data?: T;
  error?: string;
  message?: string;
...
export class TransferService {
  ...
  static async createTransfer(
    ...
    requestingUser: any
  ): Promise<TransferServiceResponse<{ transfer: TransferResponse }>> {
    try {
      ...
      if (populatedTransfer.success && populatedTransfer.data) {
        return {
          success: true,
          data: { transfer: populatedTransfer.data.transfer },
          message: 'Transfer created successfully'
        };
      } else {
        return {
          success: false,
          error: 'Failed to retrieve created transfer'
        };
      }
   ...
   * Get transfer by ID
   */
  static async getTransferById(transferId: string): Promise<TransferServiceResponse<{ transfer: TransferResponse }>> {
    try {
      // DatabaseService handles connection automatically
      ...
      if (transfer) {
        const transferResponse = this.mapTransferToResponse(transfer);
        return {
          success: true,
  ...
  ): Promise<TransferServiceResponse<TransferListResponse>> {
    try {
      // DatabaseService handles connection automatically
  ...
  static async getTransferStats(
    dateFrom?: Date,
    dateTo?: Date
  ): Promise<TransferServiceResponse<{ stats: TransferStats }>> {
    try {
      // DatabaseService handles connection automatically
      ...
      const averageCompletionTime = completedTransfers.length > 0
        ? completedTransfers.reduce((sum: number, t: any) => sum + (t.actualDuration || 0), 0) / completedTransfers.length
        : undefined;

...
import { AuditService, TransferAuditContext } from '@/lib/audit';
import { 
  createEvent,
  createTransferCreatedEvent as createTransferCreatedEventUtil,
  createStatusChangeEvent as createStatusChangeEventUtil,
  createApprovalEvent as createApprovalEventUtil,
  createRejectionEvent as createRejectionEventUtil,
  transformAuditLogToTimelineItem,
  mapTimelineTypesToAuditActions,
  applyTimelineFilters
} from '../utils';

// Service-level response types
export interface TimelineServiceResponse<T = any> {
  success: boolean;
  data?: T;
  error?: string;
  message?: string;
...
export class TimelineService {
   ...
   */
  // generateEventId moved to utils
  ...
  static async createEventWithAudit(
    ...
    const timelineEvent = createEvent(data);
  ...
  private static async logToAuditSystem(
    ...
    try {
      ...
      const transferContext: TransferAuditContext = {
        ...
        ...(timelineEvent.metadata?.reason && { reason: timelineEvent.metadata.reason }),
        details: {
   ...
   */
  private static _mapTimelineTypeToAuditCategory(timelineType: TimelineEventType): AuditCategory {
   ...
   */
  private static assessRiskLevel(timelineType: TimelineEventType, _metadata?: any): RiskLevel {
   ...
   */
  private static _getSecurityFlags(timelineType: TimelineEventType): string[] {
    const flags: string[] = [];
   ...
   */
  private static _calculateRiskScore(timelineType: TimelineEventType, metadata?: any): number {
  ...
  static createTransferCreatedEvent(
    ...
    return createTransferCreatedEventUtil(
      actor, 
      transferData.transferId,
      `${transferData.patientInfo.firstName} ${transferData.patientInfo.lastName}`,
      transferData.fromHospitalName,
      transferData.toHospitalName
    );
  }
  ...
  static createStatusChangeEvent(
    ...
    return createStatusChangeEventUtil(actor, oldStatus, newStatus, reason);
  }
  ...
  static createApprovalEvent(
    ...
    return createApprovalEventUtil(actor, reason);
  }
  ...
  static createRejectionEvent(
    ...
    return createRejectionEventUtil(actor, reason);
  }
  ...
  /**
   * Get timeline for a specific transfer from audit logs with pagination
   */
  static async getTimelineForTransfer(
    transferId: string, 
    options: TimelineQueryOptions = {}
  ): Promise<TimelineServiceResponse<TimelineResponse>> {
    try {
      const {
        page = 1,
        limit = 50,
  ...
  static async getTimelineForUser(
    ...
    try {
      ...
      if (options.filters?.kind && options.filters?.kind.length > 0) {
        query.action = { $in: mapTimelineTypesToAuditActions([options.filters.kind] as any) };
      }
   ...
   */
  static async getTimelineStats(transferId: string): Promise<TimelineServiceResponse<{ stats: TimelineStats }>> {
    try {
      const query = {
        'targetResource.type': 'transfer',
        'targetResource.id': transferId
      ...
      // Get last activity
      const lastActivity = auditLogs.length > 0 ? auditLogs[0]?.timestamp || new Date() : new Date();

...
import { TransferResponse } from '../../../types/transfers/transfer.types';
...
export class TransferNotificationHandler implements TransferEventHandler {
  ...
  private async handleTransferCreated(eventData: TransferEventData): Promise<void> {
    ...
    const notificationData: TransferNotificationData = {
      ...
      transfer: {
        ...
        ...(transfer.scheduledDate && { scheduledDate: new Date(transfer.scheduledDate) })
      },
      requestedBy: {
  ...
  private async createPersistentNotification(
    _notificationData: TransferNotificationData,
    _targetRole?: 'employee' | 'manager' | 'admin',
    _targetUsers?: string[]
  ): Promise<void> {
    try {
      // DatabaseService handles connection automatically

      // TODO: Implement notification creation when Notification model is available

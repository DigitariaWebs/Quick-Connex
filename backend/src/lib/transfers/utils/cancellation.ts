 ...
 */
export function getRemainingCancellationTime(transfer: ITransfer | TransferLike): number {
  if (!transfer.acceptedAt) {
    return 0;
  }
 ...
 */
export function getRemainingCancellationTimeString(transfer: ITransfer | TransferLike): string {
 ...
 */
export function isCancellationWindowExpiring(transfer: ITransfer | TransferLike): boolean {
  const remainingMs = getRemainingCancellationTime(transfer);
